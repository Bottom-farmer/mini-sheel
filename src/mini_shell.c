...
#define SHEEL_TAG_SIZE 6

static void show_prompt(void)
{
    outputchar(outputchar_arg, '\r');
    outputchar(outputchar_arg, 'm');
    outputchar(outputchar_arg, 'n');
    outputchar(outputchar_arg, 's');
...
static void browse_history_up(struct shell_history *history, struct shell_input *input)
{
    ...
    {
        for(i = 0; i < (history->max_len + SHEEL_TAG_SIZE) * 2; i++)
        {
            if(i < (history->max_len + SHEEL_TAG_SIZE))
            {
                outputchar(outputchar_arg, ' ');
            }
            else
    ...
    if(state == STATE_NORMAL)
    {
        ...
        if(ch == '\b' || ch == 127)
        {
            if(input->pos > 0)
            {
                ...
                s_printf("\b%s \b", &input->buffer[input->pos - 1]);
...
static int hexdump(int argc, char **argv)
{
    ...
    for(i = 0; i < len; i += 16)
    {
        ...
        for(j = 0; j < 16; j++)
        {
            if(i + j < len)
            {
                val = *((unsigned char *)(unsigned long long)(start_base + i + j));
                s_printf("%02X ", val);
